...
# def plot_img_and_mask(step, img, output, mask, name):
#     fig = plt.figure(figsize=(10, 4))  # figsize的单位是inch
#     a = fig.add_subplot(1, 3, 1)
...
#
#     # 画出大标题
#     # t = 'IOU' + '_' + str(step) + ':' + str(score.item())
#     # plt.text(0, 190, t, fontsize=15, verticalalignment="bottom", horizontalalignment="center")
#
#     # 保存图像
#     # s = 'img_' + str(step)
#         # plt.savefig('./glaucoma/epoch800/img_plt.png')
#     ss = os.path.join('/home/chenxiaojing/PycharmProjects/CENet/results/seg_result_save', name)
#     plt.savefig(ss + '.png')
#
#     # show
#     plt.show()

def plot_img_and_mask(step, img, output, mask,score):
    fig = plt.figure(figsize=(10, 4))  # figsize的单位是inch
    a = fig.add_subplot(1, 3, 1)
    a.set_title('Input image')
    img = tensor2im(img)
    ...
    s = 'img_' + str(step)
        # plt.savefig('./glaucoma/epoch800/img_plt.png')
    ss = os.path.join('/home/chenxiaojing/PycharmProjects/CENet/results/optic_seg')
    plt.savefig(ss + '.png')
